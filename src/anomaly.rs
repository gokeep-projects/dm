...
pub fn rule_catalog() -> Vec<serde_json::Value> {
    const RULES: &[(
        ...
        // ============ 堆栈分析专属规则（jstack/thread dump 维度） ============
        (
            "stack.deadlock.error",
            "堆栈分析",
            "error",
            "检测到线程死锁",
            "java.thread.deadlock",
            "jstack 出现 Found N deadlocks 且 N >= 1",
            "1",
            "线程死锁会导致相关业务接口完全挂起，常见于锁顺序不一致或 synchronized + Lock 混用，需立刻排查堆栈中的等待链。",
            &["java.thread_dump.deadlocks", "java.thread_dump.lock_chain"],
            &[
        ...
        (
            ...
            "BLOCKED 线程数 / 总线程数 >= 30%",
            "30",
            "BLOCKED 线程占比偏高说明存在锁竞争热点，多为高 QPS 接口的同步块、缓存击穿或分布式锁等待。",
            &["java.thread_dump.blocked_count", "java.thread_dump.total_count"],
            &[
                "jstack -l <pid> | awk '/BLOCKED/{c++} END{print c}'",
                "jstack <pid> | grep BLOCKED | sort | uniq -c | sort -rn | head",
            ],
        ),
        (
            "stack.waiting.warning",
            "堆栈分析",
            "warn",
            "WAITING 线程聚集在单一锁/资源",
            "java.thread.waiting",
            "WAITING 线程聚集在单一 monitor/condition >= 20 个",
            "20",
            "大量线程等待同一资源通常是连接池打满、慢 SQL、远程调用阻塞的表现，应结合堆栈顶部方法定位瓶颈。",
            &["java.thread_dump.waiting_groups"],
            &[
                "jstack <pid> | grep -E 'WAITING' | grep -E 'on|parking' | head -30",
                "jcmd <pid> Thread.print | grep -A 5 'java.util.concurrent' | head -50",
            ],
        ),
        (
            "stack.cpu-high.error",
            "堆栈分析",
            "error",
            "CPU 飙高且线程集中在同一调用栈",
            "java.thread.cpu",
            ">= 3 个线程 CPU > 80% 且堆栈顶部方法相同",
            "3",
            "多线程同时进入相同热点方法会耗尽 CPU 资源，常见于死循环、频繁正则编译或低效的序列化逻辑。",
            &["java.thread.top_cpu", "java.thread.stack_top"],
            &[
                "top -H -p <pid> -o %CPU | head -10",
                "jstack <pid> | grep -A 8 '^\"' | head -80",
            ],
        ),
        (
            "stack.fullgc.error",
            ...
            "频繁 Full GC",
            "java.gc.full",
            "5 分钟内 Full GC 次数 >= 3 或单次停顿 >= 1s",
            "3",
            "频繁 Full GC 通常由内存泄漏、大对象直接进入老年代或元空间不足引起，最终会触发 OOM。",
            &["java.gc.full_count", "java.gc.full_pause_ms"],
            &[
                "jstat -gc <pid> 1s 10",
                "jcmd <pid> GC.heap_info",
                "jcmd <pid> VM.flags | grep -E 'Metaspace|Heap'",
            ],
        ),
        (
            "stack.oom.error",
            ...
            "java.memory.oom",
            "日志或堆栈出现 OutOfMemoryError / Java heap space / Metaspace",
            "1",
            "OOM 会导致业务中断并需要重启恢复，需立刻保留堆转储文件后定位泄漏对象。",
            &["java.log.oom", "java.heap_dump.path"],
            &[
                "jcmd <pid> GC.heap_dump /tmp/heapdump.hprof",
                "grep -A 30 'OutOfMemoryError' <log_path> | head -50",
            ],
        ),
        (
            "stack.leak.warning",
            ...
            "疑似堆内存泄漏",
            "java.memory.leak",
            "老年代持续增长且 Full GC 后回收率 < 30%",
            "30",
            "老年代回收率持续走低说明有对象无法被回收，常见为静态集合缓存未清理、监听器未注销或 ThreadLocal 未 remove。",
            &["java.gc.old_gen", "java.gc.old_gen_reclaimed"],
            &[
        ...
        (
            "stack.thread-leak.warning",
            "堆栈分析",
            "warn",
            "线程数异常增长",
            "java.thread.count",
            "线程数 >= 500 或较启动基线增长 >= 2 倍",
            "500",
            "线程数持续上涨而不释放通常代表线程池未关闭、异步任务未 await 或第三方 SDK 泄漏线程。",
            &["java.thread.count", "java.thread.names"],
            &[
                "jstack <pid> | grep '^\"' | awk '{print $1}' | sort | uniq -c | sort -rn | head",
